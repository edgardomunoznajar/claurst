...
/// 2. `<SIMON_REMOTE_MEMORY_DIR>/projects/<sanitized-root>/memory/`
///    when `SIMON_REMOTE_MEMORY_DIR` is set.
/// 3. `~/.simon/projects/<sanitized-root>/memory/` (default).
pub fn auto_memory_path(project_root: &Path) -> PathBuf {
    ...
    let memory_base = std::env::var("SIMON_REMOTE_MEMORY_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            dirs::home_dir()
...
/// 1. `SIMON_DISABLE_AUTO_MEMORY` — truthy → OFF, falsy (but defined) → ON.
/// 2. `SIMON_SIMPLE` (--bare) → OFF.
/// 3. Remote mode without `SIMON_REMOTE_MEMORY_DIR` → OFF.
...
pub fn is_auto_memory_enabled(settings_enabled: Option<bool>) -> bool {
    if let Ok(val) = std::env::var("SIMON_DISABLE_AUTO_MEMORY") {
    ...
    if std::env::var("SIMON_SIMPLE").is_ok() {
        return false;
    }

    if std::env::var("SIMON_REMOTE").is_ok()
        && std::env::var("SIMON_REMOTE_MEMORY_DIR").is_err()
    {
        return false;
    }
