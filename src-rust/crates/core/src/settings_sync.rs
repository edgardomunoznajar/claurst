...
//
// Syncs user settings and AGENTS.md memory files between a local Simon
// installation and claude.ai via:
//   - Upload (interactive CLI, fire-and-forget at startup)
//   - Download (CCR / SIMON_REMOTE=1, blocking before plugin load)
//
...
pub const SYNC_KEY_USER_SETTINGS: &str = "~/.simon/settings.json";
/// Canonical sync key for the global user memory file.
pub const SYNC_KEY_USER_MEMORY: &str = "~/.simon/AGENTS.md";
...
pub fn sync_key_project_settings(project_id: &str) -> String {
    format!("projects/{project_id}/.simon/settings.local.json")
}
...
pub async fn collect_local_entries(project_id: Option<&str>) -> HashMap<String, String> {
    ...
    if let Some(pid) = project_id {
        ...
        let local_settings = cwd.join(".simon").join("settings.local.json");
...
/// Return the ~/.simon directory.
fn claude_config_dir() -> PathBuf {
    dirs::home_dir()
        .map(|h| h.join(".simon"))
        .unwrap_or_else(|| PathBuf::from(".simon"))
}
...
mod tests {
    ...
    #[test]
    fn test_sync_keys() {
        assert_eq!(SYNC_KEY_USER_SETTINGS, "~/.simon/settings.json");
        assert_eq!(SYNC_KEY_USER_MEMORY, "~/.simon/AGENTS.md");
        assert_eq!(
            sync_key_project_settings("abc123"),
            "projects/abc123/.simon/settings.local.json"
        );
        assert_eq!(
