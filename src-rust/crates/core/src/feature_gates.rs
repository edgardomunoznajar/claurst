...
///
/// Reads `SIMON_FEATURE_<NORMALIZED_NAME>` and returns `true` when the
...
pub fn is_feature_enabled(gate_name: &str) -> bool {
    let key = format!("SIMON_FEATURE_{}", normalize_name(gate_name));
...
///
/// Reads `SIMON_DYNAMIC_CONFIG_<NORMALIZED_NAME>`.  If the variable is
...
pub fn get_dynamic_config<T: DeserializeOwned>(name: &str, default: T) -> T {
    let key = format!("SIMON_DYNAMIC_CONFIG_{}", normalize_name(name));
