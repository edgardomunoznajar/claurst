...
// They will NOT work for Simon — Anthropic's auth server will reject or misattribute requests.
// Users should use an API key from console.anthropic.com instead.
// To use OAuth, Simon would need its own registered OAuth application with Anthropic.
pub const PROD_OAUTH: OAuthConfig = OAuthConfig {
    ...
    client_id: "9d1c250a-e61b-44d9-88ed-5944d1962f5e", // Anthropic's Claude Code — will not work for Simon
    oauth_file_suffix: "",
...
pub const STAGING_OAUTH: OAuthConfig = OAuthConfig {
    ...
    client_id: "22422756-60c9-4084-8eb7-27705fd5cf9a", // Anthropic's Claude Code staging — will not work for Simon
...
/// OpenAI Codex OAuth tokens, persisted to ~/.simon/codex_tokens.json
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Path to the Codex tokens file (~/.simon/codex_tokens.json)
fn codex_tokens_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|h| h.join(".simon").join("codex_tokens.json"))
}

/// Save Codex OAuth tokens to ~/.simon/codex_tokens.json
...
/// Load Codex OAuth tokens from ~/.simon/codex_tokens.json
...
/// or by setting `SIMON_USE_OPENAI=1` with a manually stored token.
