...
- User:    ~/.simon/settings.json
- Project: .simon/settings.json
- Local:   .simon/settings.local.json
...
6. Report: what was verified, what passed, what failed or is uncertain."#,
    ...
    BundledSkill {
        name: "update-config",
        description: "Configure Simon settings (hooks, permissions, env vars, behaviours) via settings.json.",
...
Modify Simon configuration by updating settings.json files.

## Settings File Locations

| File | Scope | Use For |
|------|-------|---------|
| `~/.simon/settings.json` | Global | Personal preferences for all projects |
| `.simon/settings.json` | Project | Team-wide hooks, permissions, plugins |
| `.simon/settings.local.json` | Project (local) | Personal overrides for this project |
