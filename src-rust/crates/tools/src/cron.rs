...
// Durable tasks are persisted to `~/.simon/scheduled_tasks.json`.
//
...
/// Path to `~/.simon/scheduled_tasks.json`.
fn scheduled_tasks_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".simon").join("scheduled_tasks.json"))
}
...
async fn ensure_store_loaded() {
    ...
    *init = true;
    ...
    // Load from ~/.simon/scheduled_tasks.json if it exists.
...
impl Tool for CronCreateTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "durable": {
                    "type": "boolean",
                    "description": "true = persist to .simon/scheduled_tasks.json; false (default) = session only"
                }
            },
    ...
    async fn execute(&self, input: Value, _ctx: &ToolContext) -> ToolResult {
        ...
        // Persist to ~/.simon/scheduled_tasks.json for durable tasks.
        if params.durable {
        ...
        let where_note = if params.durable {
            "Persisted to ~/.simon/scheduled_tasks.json"
        } else {
...
/// Persist all durable tasks to `~/.simon/scheduled_tasks.json`.
