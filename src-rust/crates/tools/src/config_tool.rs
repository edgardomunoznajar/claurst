// ConfigTool: get or set Simon configuration settings at runtime.
//
// Reads from and persists to ~/.simon/settings.json.
...
impl Tool for ConfigTool {
    ...
    fn description(&self) -> &str {
        "Get or set Simon configuration settings. Omit 'value' to read the current value. \
         Supported settings: model, max_tokens, verbose, permission_mode, auto_compact. \
         Changes persist to ~/.simon/settings.json."
    }
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let mut settings = match simon_core::config::Settings::load().await {
            Ok(s) => s,
...
fn permission_mode_str(mode: &simon_core::config::PermissionMode) -> &'static str {
    use simon_core::config::PermissionMode;
    match mode {
