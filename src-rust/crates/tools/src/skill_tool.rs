...
//   <project>/.simon/commands/<name>.md
//   ~/.simon/commands/<name>.md
//
...
impl Tool for SkillTool {
    ...
    fn description(&self) -> &str {
        "Execute a skill (custom prompt template) by name. \
         Skills are .md files in .simon/commands/ or ~/.simon/commands/. \
         Use skill=\"list\" to discover available skills. \
...
fn skill_search_dirs(ctx: &ToolContext) -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        dirs.push(home.join(".simon").join("commands"));
    }
    dirs
}
...
async fn list_skills(dirs: &[PathBuf]) -> ToolResult {
    ...
    if total == 0 {
        return ToolResult::success(
            "No skills found. Create .md files in .simon/commands/ to define skills.\n\
             Example: .simon/commands/review.md"
                .to_string(),
        );
    }
