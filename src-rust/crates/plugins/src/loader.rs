...
/// 1. `~/.simon/plugins/<name>/`  — user-global plugins
/// 2. `<project>/.simon/plugins/<name>/`  — project-local plugins
...
/// Return the default user-level plugins directory: `~/.simon/plugins`.
pub fn default_user_plugins_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".simon").join("plugins"))
}

/// Return the project-level plugins directory: `<project>/.simon/plugins`.
pub fn project_plugins_dir(project_dir: &Path) -> PathBuf {
    project_dir.join(".simon").join("plugins")
}
