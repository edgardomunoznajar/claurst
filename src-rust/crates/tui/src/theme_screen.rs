...
use crate::overlays::{
    begin_modal_frame, modal_header_line_area, render_modal_title_frame, SIMON_ACCENT,
...
fn builtin_themes() -> Vec<ThemeOption> {
    vec![
        ThemeOption {
            ...
            description: "Simon default — dark background, cyan accents".to_string(),
...
pub fn render_theme_screen(frame: &mut Frame, screen: &ThemeScreen, area: Rect) {
    ...
    for (i, theme) in screen.themes.iter().enumerate() {
        ...
        let bg = if is_selected { SIMON_ACCENT } else { SIMON_PANEL_BG };
        let fg = if is_selected { Color::White } else { SIMON_TEXT };
        let desc_fg = if is_selected { Color::Rgb(248, 220, 236) } else { SIMON_MUTED };
    ...
    frame.render_widget(
        Paragraph::new(lines).style(Style::default().bg(SIMON_PANEL_BG)),
        layout.body_area,
    );
    frame.render_widget(
        Paragraph::new(Line::from(vec![Span::styled(
