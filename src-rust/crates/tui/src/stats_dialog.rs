...
//! Data source: ~/.simon/stats.jsonl (append-only per-turn usage log)
...
/// A single entry in ~/.simon/stats.jsonl
...
/// Load and aggregate stats from ~/.simon/stats.jsonl
pub fn load_stats() -> AggregatedStats {
    let path = dirs::home_dir()
        .map(|h| h.join(".simon").join("stats.jsonl"))
        .unwrap_or_default();
...
fn tab_span(label: &str, active: bool) -> Span<'static> {
    if active {
        ...
        Span::styled(label.to_string(), Style::default().fg(SIMON_MUTED))
    }
}
