...
pub const SIMON_ACCENT: Color = Color::Rgb(233, 30, 99);
pub const SIMON_PANEL_BG: Color = Color::Rgb(20, 20, 28);
pub const SIMON_PANEL_BORDER: Color = Color::Rgb(72, 72, 80);
pub const SIMON_TEXT: Color = Color::Rgb(235, 235, 240);
pub const SIMON_MUTED: Color = Color::Rgb(110, 110, 118);
pub const SIMON_OVERLAY_BG: Color = Color::Rgb(10, 10, 14);
    ...
    for y in area.y..area.y + area.height {
        for x in area.x..area.x + area.width {
            if let Some(cell) = buf.cell_mut((x, y)) {
                cell.set_bg(SIMON_OVERLAY_BG);
                cell.set_fg(SIMON_MUTED);
            }
        }
    }
}
...
pub fn render_dialog_bg_buf(buf: &mut Buffer, area: Rect) {
    for y in area.y..area.y + area.height {
        for x in area.x..area.x + area.width {
            if let Some(cell) = buf.cell_mut((x, y)) {
                cell.set_char(' ');
                cell.set_bg(SIMON_PANEL_BG);
                cell.set_fg(SIMON_TEXT);
            }
        }
    }
}
...
pub fn render_modal_title_frame(frame: &mut Frame, area: Rect, title: &str, right_hint: &str) {
    ...
    let line = Line::from(vec![
        ...
        Span::styled(" ".repeat(padding), Style::default().fg(SIMON_TEXT)),
        Span::styled(
...
pub fn render_help_overlay(frame: &mut Frame, overlay: &HelpOverlay, area: Rect) {
    ...
    use simon_core::constants::APP_VERSION;
    ...
    frame.render_widget(
        Paragraph::new(left_lines)
            ...
            .style(Style::default().bg(SIMON_PANEL_BG)),
        col_chunks[0],
    );

    // ─── Center divider ────────────────────────────────────────────────────
    let divider_lines: Vec<Line<'static>> = (0..content_area.height)
        .map(|_| Line::from(Span::styled("\u{2502}", Style::default().fg(SIMON_MUTED))))
        .collect();
    ...
    for entry in &filtered {
        ...
        right_lines.push(Line::from(vec![
            ...
            Span::styled(aliases_text, Style::default().fg(SIMON_MUTED)),
            Span::raw("  "),
            Span::styled(entry.description.clone(), Style::default().fg(SIMON_MUTED)),
        ]));
    }
...
// ---------------------------------------------------------------------------
// Pinned-entry persistence  (~/.simon/history_pins.json)
// ---------------------------------------------------------------------------
...
/// Load the set of pinned entry texts from `~/.simon/history_pins.json`.
...
/// Persist `pinned_texts` to `~/.simon/history_pins.json`.
...
impl HistorySearchOverlay {
    ...
    ///
    /// Pinned state is loaded from `~/.simon/history_pins.json` and applied
    ...
    ///
    /// Persists the updated pin set to `~/.simon/history_pins.json` and
...
fn kb_line<'a>(key: &str, desc: &str) -> Line<'a> {
    Line::from(vec![
        ...
        Span::styled(desc.to_string(), Style::default().fg(SIMON_MUTED)),
    ])
}
...
mod tests {
    ...
    fn modal_search_line_separates_leading_space_from_cursor() {
        let line = modal_search_line("", "Search", SIMON_MUTED, SIMON_TEXT);
