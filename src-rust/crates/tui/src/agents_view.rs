...
use crate::overlays::{
    begin_modal_buf, modal_header_line_area, render_modal_title_buf, SIMON_ACCENT,
    SIMON_MUTED, SIMON_PANEL_BG, SIMON_TEXT,
};
...
/// A defined agent (from .simon/agents/*.md or plugin).
#[derive(Debug, Clone)]
...
/// Load agent definitions from `.simon/agents/` in project root and home dir.
pub fn load_agent_definitions(project_root: &std::path::Path) -> Vec<AgentDefinition> {
    let mut defs = Vec::new();
    let dirs = [
        dirs::home_dir().map(|h| h.join(".simon").join("agents")),
        Some(project_root.join(".simon").join("agents")),
    ];
...
pub fn render_agents_menu(state: &AgentsMenuState, area: Rect, buf: &mut Buffer) {
    ...
    Paragraph::new(Line::from(vec![Span::styled(
        footer,
        Style::default().fg(SIMON_MUTED).add_modifier(Modifier::ITALIC),
    )]))
    ...
    if !state.active_agents.is_empty() {
        ...
        for agent in state.active_agents.iter().take(3) {
            lines.push(Line::from(vec![
                Span::styled(" ", Style::default().fg(SIMON_MUTED)),
                Span::styled(agent.name.clone(), Style::default().fg(SIMON_TEXT)),
                Span::styled(
...
fn render_agent_detail(def: &AgentDefinition, area: Rect, buf: &mut Buffer) {
    let mut lines = Vec::new();
    lines.push(Line::from(vec![
        Span::styled(" Name       ", Style::default().fg(SIMON_MUTED)),
        Span::styled(
            def.name.clone(),
            Style::default()
                .fg(SIMON_TEXT)
    ...
    lines.push(Line::from(vec![
        Span::styled(" Model      ", Style::default().fg(SIMON_MUTED)),
        Span::raw(def.model.as_deref().unwrap_or("default").to_string()),
    ]));
    if let Some(mem) = &def.memory_scope {
        lines.push(Line::from(vec![
            Span::styled(" Memory     ", Style::default().fg(SIMON_MUTED)),
            Span::raw(mem.clone()),
        ]));
    }
    if !def.tools.is_empty() {
        lines.push(Line::from(vec![
            Span::styled(" Tools      ", Style::default().fg(SIMON_MUTED)),
            Span::raw(def.tools.join(", ")),
        ]));
    } else {
        lines.push(Line::from(vec![
            Span::styled(" Tools      ", Style::default().fg(SIMON_MUTED)),
            Span::styled("All tools", Style::default().fg(SIMON_MUTED)),
        ]));
    }
...
fn render_agent_editor(state: &AgentsMenuState, area: Rect, buf: &mut Buffer) {
    ...
    let selected_style = Style::default()
        ...
        .bg(SIMON_ACCENT)
        .add_modifier(Modifier::BOLD);
    let normal_style = Style::default().fg(SIMON_TEXT);
...
fn agent_list_row(title: String, meta: String, selected: bool, width: u16) -> Line<'static> {
    let bg = if selected { SIMON_ACCENT } else { SIMON_PANEL_BG };
    let title_style = if selected {
        Style::default().fg(Color::White).bg(bg).add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(SIMON_TEXT).bg(bg)
    };
    let meta_style = if selected {
        Style::default().fg(Color::Rgb(248, 220, 236)).bg(bg)
    } else {
        Style::default().fg(SIMON_MUTED).bg(bg)
    };
    let mut spans = vec![
