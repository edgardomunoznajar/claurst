...
//
// The bridge connects the local Simon CLI to the claude.ai web UI,
...
impl BridgeConfig {
    ...
    /// - `SIMON_BRIDGE_URL` — overrides `server_url` and sets `enabled = true`
    /// - `SIMON_BRIDGE_TOKEN` / `CLAUDE_BRIDGE_OAUTH_TOKEN` — sets `session_token`
    /// - `CLAUDE_BRIDGE_BASE_URL` — alternative URL override (ant-only dev override)
    pub fn from_env() -> Self {
        let mut config = Self::default();

        // URL override (sets enabled implicitly)
        if let Ok(url) = std::env::var("SIMON_BRIDGE_URL")
        ...
        // Token override
        if let Ok(token) = std::env::var("SIMON_BRIDGE_TOKEN")
...
/// 1. `SIMON_BRIDGE_TOKEN` environment variable
...
/// match simon_bridge::start_bridge_session(None).await {
...
pub async fn start_bridge_session(
    ...
    let server_url = std::env::var("SIMON_BRIDGE_URL")
        .or_else(|_| std::env::var("CLAUDE_BRIDGE_BASE_URL"))
